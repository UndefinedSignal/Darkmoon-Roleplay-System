...
<p>Менестрель (сокр. МС) - роль, открывающая для владельца базовый функционал для ведения собственных сюжетов.</p>
</body></html>]]

RPSCoreFramework.Literature.DarkmoonMinstrel1 = [[
Роль Менестреля можно получить, активировав Жетон Менестреля. Жетон, в свою очередь, можно получить двумя способами. Первый это купить его у Жадного Торговца. Второй это оставить заявку на сайте в разделе получения роли Менестреля. Стоит сказать, что к кандидатам на получение статуса Менестреля через заявку предъявляются требования, такие как, наличие компании, для которой менестрель собирается организовывать сюжет.
|n|n
Функционал Менестреля позволяет ставить временных НИП (неиграбельные персонажи) и некоторые типы объектов (ГО) для проведения собственных сюжетов. Также функционал позволяет взаимодействовать с размещенными менестрелем НИПами, например, задавать им эмоции или заставлять их бродить.
]]
